...
#define CS_HIGH() { HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, SET); }
#define CS_LOW()  { HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, RESET); }
...
//  SPI1->CR1 |= SPI_CR1_SPE;
  ...
  while (btr >= 32) {
    HAL_SPI_TransmitReceive(&hspi1, (uint8_t*)dmybuf, (uint8_t*)buff, 32, 5000);
    btr -= 32;
    buff += 32;
  }
  
  if (btr) {
    HAL_SPI_TransmitReceive(&hspi1, (uint8_t*)dmybuf, (uint8_t*)buff, btr, 5000);
  }
